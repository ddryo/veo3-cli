...
| FR-013 | エラーハンドリング | API エラー（400, 403, 429, 500, 503）・ネットワークエラー・タイムアウトエラーを分類し、対処方法付きのメッセージで表示する | Must | ✓ |
...
```
cli.ts
  │  ユーザー入力をパース（commander）
  │  トップレベル try-catch でエラーを捕捉
  ▼
config.ts ← .env
...
errors.ts ← cli.ts のトップレベル try-catch から呼び出し
  │  HTTP ステータスコード別分類（400, 403, 429, 500, 503）
  │  ネットワークエラー・タイムアウトエラーの判定
  │  ユーザー向けメッセージの生成
```
...
| NFR-005 | ポーリング間隔 | 動画生成のポーリング間隔を 10 秒に設定する |
